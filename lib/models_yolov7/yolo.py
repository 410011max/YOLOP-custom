...
class Model(nn.Module):
    def __init__(self, cfg='yolor-csp-c.yaml', ch=3, nc=None, anchors=None):  # model, input channels, number of classes
        ...
        m = self.model[self.detector_index]  # Detect()
        if isinstance(m, Detect):
            s = 256  # 2x min stride
             # NOTE: brian1009 Fix bug
            with torch.no_grad():
...
def parse_model(d, ch):  # model_dict, input_channels(3)
    ...
    for i, (f, n, m, args) in enumerate(d['backbone'] + d['head'] + d['head_drive_area'] + d['head_line']):  # from, number, module, args
        #print(f"currently handle id:{i}, module:{m}")
        ...
        if m in [nn.Conv2d, Conv, RobustConv, RobustConv2, DWConv, GhostConv, RepConv, RepConv_OREPA, DownC, 
                 ...
                 Bottleneck, BottleneckCSPA, BottleneckCSPB, BottleneckCSPC, BottleneckCSPD,
