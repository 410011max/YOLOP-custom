...
[ -1, Conv, [8, 6, 3, 1]], #33 Driving area & Lane line segmentation head

# Move and combine Lane line segmentation head to Driving area segmentation head
...
class MCnet(nn.Module):
    ...
    def forward(self, x):
        ...
        for i, block in enumerate(self.model):
            ...
            x = block(x)
            if i == self.seg_out_idx:     # save driving area & lane line segment result
                m=nn.Softmax(dim=1)
                out.append(m(x))
                out.append(m(x))
...
if __name__ == "__main__":
    ...
    detects, DA_LL_seg = model_out
    print('\nDetects:')
    for det in detects:
        print(det.shape)
    print('\nSegmentation:')
    print(DA_LL_seg.shape)
